...
export default function OrderPredCard({ orderpred, onDelete }) { // these 2 are props received
  ...
  return (
    <div 
      className = "p-4 rounded-lg border shadow-sm transition-all bg-white border-gray-200 text-gray-800"
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div>
          <strong className="text-base font-semibold text-gray-900">
            #{orderpred.id} - {orderpred.product_obj?.name}
          </strong>
        </div>
        
        <div className="flex items-center gap-3 mt-2 sm:mt-0">
          <Link to={`/orderpred/history/${orderpred.id}`}> {/* matching what defined in App.jsx, not django */}
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 text-gray-800 space-y-4">
            ...
            <p className="text-sm text-gray-600">Are you sure you want to delete this orderpred?</p>
