...
export default function SalesCard({ sale, onDelete }) { // receive sale object and onDelete handler
  ...
  const handleDelete = async () => {
    try {
      ...
      await API.delete(`/inventory/sales/${sale.id}`); // 2. Send DELETE request
      onDelete(sale.id); // 3. Remove item from parent state
  ...
  // Optional: Formats ISO date to a readable string (e.g., "Sep 19, 2026")
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div>
          <strong className="text-base font-semibold text-gray-900">
            #{sale.id} - {sale.product_obj?.name} {/* i use custom serializer to get product obj */}
          </strong>
          <div className="text-sm text-gray-500 mt-1">
            <span>Sold: <span className="font-medium text-gray-800">{sale.quantity_sold} units</span></span>
            <span className="mx-2">—</span>
            <span>Revenue: <span className="font-medium text-emerald-600">${sale.total_revenue}</span></span>
            <span className="mx-2">—</span>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 text-gray-800 space-y-4">
            ...
            <p className="text-sm text-gray-600">Are you sure you want to delete this sale record?</p>
